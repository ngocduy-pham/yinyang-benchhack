...
final class YYTransformer[C <: Context, T](
  ...
  def apply[T](block: c.Expr[T]): c.Expr[T] = {
    ...
    log(s"\nrequired: ${requiredVariables mkString ", "}\n")
    ...
    val dslTree = dslInstance(dslClassPre) match {
       ...
       */
        ...
        /*val guard =
          if (noGuard) c parse s"val $nameDSLProgram = $nameRecompile()"
          else c parse s"""val $nameDSLProgram = 
        ...
        val guard =
          if (noGuard) c parse s"val $nameDSLProgram = $nameRecompile()"
            ...
            $nameCompileStorage.checkAndUpdate[Int](${new scala.util.Random().nextInt}, $nameCurrent, () => () => false)""" match {
            case ValDef(modes, name, tp, Apply(TypeApply(check, _), args)) ⇒
              ValDef(modes, name, tp, Apply(TypeApply(check, List(functionTypeTree)), args))
          }

        val DSL = if (noGuard) c parse s"$nameDSLProgram.apply(${args(holes)})" else (c literal true).tree
