...
object YinYangBuild extends Build {
  ...
  lazy val defaults = Defaults.defaultSettings ++ formatSettings ++ Seq(
    ...
    scalaHome := Some(file("E:/Tool/sbt/.sbt/boot/scala-2.10.1-RC1")),
  ...
  lazy val benchhack       = Project(id = "bench-hack",       base = file("benchhack"), settings = defaults) dependsOn(framework, vector_dsl)
}
